-- Таблица сертификатов из смарт-процесса (entityTypeId=131)
CREATE TABLE IF NOT EXISTS certificates (
  id                   BIGINT PRIMARY KEY,              -- CRM item id
  xml_id               TEXT,
  title                TEXT NOT NULL,
  ...
  opened               BOOLEAN NOT NULL,                -- "Y"/"N" -> boolean
  ...
  is_manual_opportunity BOOLEAN NOT NULL,               -- "Y"/"N" -> boolean
...
-- Индексы под частые выборки/поиск
...
-- Документы сертификатов (UF_DOCUMENTS)
CREATE TABLE IF NOT EXISTS certificate_documents (
  id              BIGINT PRIMARY KEY,                   -- fileId из UF_DOCUMENTS[].id
  certificate_id  BIGINT NOT NULL REFERENCES certificates(id) ON DELETE CASCADE,
  url             TEXT NOT NULL,
  url_machine     TEXT NOT NULL,                        -- REST URL с токеном
